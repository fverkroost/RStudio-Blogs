...
Finally, we prepare a balanced subsample of 1000 images (100 per clothing category) of the training data for later on. We use the pipeline (`%>%`) in package `magrittr` together with functions from package `dplyr` to manipulate the data. As some of the models take quite long, we fit them on a subsample of the training data due to time constraints. Fitting the models on the full training dataset is preferable though.
